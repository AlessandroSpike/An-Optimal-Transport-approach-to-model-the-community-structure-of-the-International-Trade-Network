...
for i=1:size(Y,2)
    ...
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i},0:.01:max(Y{i}),'support','positive','bandwidth',.1);
    end
    
    f=f/max(f)*0.1; %normalize
    %[f,pos]=sort(f,'descend');
    %u=u(pos);
    F{i}=f;
    U{i}=u;
    % MED(:,i)=nanmedian(Y{i});
...
for i=i:size(Y,2)
    ...
    if isempty(lc) == 1
        ...
        if setX == 0
            fill([F{i}+i;flipud(i-F{i})],[U{i};flipud(U{i})],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc)
        else
